...
object ExpParser {
  ...
  def parse(sexp: SExpr): Exp = sexp match {
    ...
    case SList(List(SSym("-"), exp1, exp2)) => SubtExp(parse(exp1), parse(exp2)) // (- exp1 exp2)
    ...
    case SList(List(SSym(">="), exp1, exp2)) => GreaterEqualExp(parse(exp1), parse(exp2)) // (>= exp1 exp2)
    ...
    case SList(List(SSym("repeat"), body, untilCond)) => parse(ExpDesugar.desugar(sexp)) // (repeat body until-cond)

    case _ => throw ExpParserException(s"error in the expression '$sexp'")
  }

  /* #9 Implemente a verificação de palavras reservadas de modo que não seja possível utilizar 
  uma palavra-chave como 'while', por exemplo, como um identificador. */
